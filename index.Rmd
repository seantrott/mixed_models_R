...
We'll be using something called the **likelihood ratio test**^[Other well-known approaches include [AIC](https://en.wikipedia.org/wiki/Akaike_information_criterion) and [BIC](https://en.wikipedia.org/wiki/Bayesian_information_criterion), both of which also involve calculating the likelihood of a model.]. For more details on likelihood ratio tests, see my [tutorial on model comparison strategies more generally](https://seantrott.github.io/model_comparison/).
...
- **`Chisq`**: this is the chi-squared statistic we'll be looking up to obtain our p-value. Note that this is identical to the log-likelihood ratio $LR$ (see [Wilks' Theorem](https://en.wikipedia.org/wiki/Wilks%27_theorem)).    
...
> A model including a fixed effect of Days, as well as by-subject random slopes for the effect of Days (and by-subject random intercepts), explained significantly more variance than a reduced model omitting only the fixed effect of Days $[\chi^{2}$(`r df`) = `r chisq`, $p$ = `r round(p, 10)`$]$.
...
As in the OLS regression, the coefficient is positive: for every day of sleep deprivation, we expect participants' `Reaction` to slow by an average of `r round(coef_days, 2)` ms. The key difference is that this parameter estimate now accounts for nested variance in the data, i.e., individual differences in `Reaction` overall (random intercepts for each `Subject`), and individual differences in the effect of `Days` of sleep deprivation on `Reaction` (by-participant random slopes for `Reaction ~ Days`).
...
**Should I include random factor $X$ in the model?** If $X$ demonstrates nested structure in my dataset, then I include it. It seems preferable to me to try to account for potential nested variance, and perhaps be overly cautious, than to miss something important--i.e., I try to keep it maximal. That said, you can also use model comparison approaches to determine your random effects structure, as [Bates et al (2015)](https://arxiv.org/pdf/1506.04967.pdf) demonstrate. This can be helpful for avoiding overly/unnecessarily complex random factor specifications and the dreaded convergence errors.
